...
class BookRemoteMediator(
    ...
    override suspend fun load(
        ...
        return try {
            ...
            val prev =
                if (currentPage in (1 until response.totalPages))
                    currentPage - 1 else null
            val next =
                if (currentPage in (0 until response.totalPages - 1))
                    currentPage + 1 else null
            db.withTransaction {
                ...
                dao.insert(response.content.map { b ->
                    BookPostEntity(b).also {
                        it.routeId = routeId
                    }
                })
            }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(
        ...
        state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.bookPostId?.let { id -> remoteDao.get(id) }
        }
    ...
    private suspend fun getRemoteKeyForFirstItem(
        ...
        state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()
            ?.let { h -> remoteDao.get(h.bookPostId) }
